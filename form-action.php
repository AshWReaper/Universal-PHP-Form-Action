...
$config = [
  ...
  'forms' => [
    ...
    'contact_basic' => [
      ...
      'files' => [
        'enabled'      => true,
        'input_name'   => 'attachments', // <input name="attachments[]" multiple>
        'max_files'    => 3,
        'max_mb'       => 5,             // per file
        'allowed_mime' => ['application/pdf','image/jpeg','image/png'],
        'attach_to_mail'=> true,         // attach to outgoing email
        'persist'      => false,         // if false, delete after sending
      ],
...
// Idempotency (drop duplicate)
$idPayload = $fields;
// include minimal file meta for hash
if (!empty($_FILES) && !empty($formCfg['files']['enabled'])) {
  $input = $formCfg['files']['input_name'] ?? null;
  if ($input && isset($_FILES[$input])) {
    $norm = normalize_files_array($_FILES[$input]);
    $idPayload['__files'] = array_map(fn($f)=>[$f['name'],$f['size']], $norm);
  }
}
...
$savedFiles = [];
if (!empty($formCfg['files']['enabled'])) {
  $input = $formCfg['files']['input_name'] ?? '';
  if ($input && isset($_FILES[$input])) {
    $allowed = $formCfg['files']['allowed_mime'] ?? [];
    $maxMB   = (int)($formCfg['files']['max_mb'] ?? 5);
    $maxB    = $maxMB * 1024 * 1024;
    $maxCnt  = (int)($formCfg['files']['max_files'] ?? 1);
    $attach  = !empty($formCfg['files']['attach_to_mail']);
    $persist = !empty($formCfg['files']['persist']);

    $items   = normalize_files_array($_FILES[$input]);
    if (count($items) > $maxCnt) {
      $payload = ['ok'=>false,'message'=>"You can upload at most {$maxCnt} file(s).",'request_id'=>$rid];
      json_response($payload, $formCfg['redirect_success'] ?? null, $formCfg['redirect_error'] ?? null);
      exit;
    }
    ...
    foreach ($items as $idx => $f) {
      ...
      if ($f['error'] !== UPLOAD_ERR_OK) {
        $payload = ['ok'=>false,'message'=>"Upload error (code {$f['error']}).",'request_id'=>$rid];
        json_response($payload, $formCfg['redirect_success'] ?? null, $formCfg['redirect_error'] ?? null);
        exit;
      }
      if ($f['size'] <= 0 || $f['size'] > $maxB) {
        $payload = ['ok'=>false,'message'=>"Each file must be <= {$maxMB}MB.",'request_id'=>$rid];
        json_response($payload, $formCfg['redirect_success'] ?? null, $formCfg['redirect_error'] ?? null);
        exit;
      }
      $mime = $finfo->file($f['tmp_name']) ?: $f['type'];
      if ($allowed && !in_array($mime, $allowed, true)) {
        $payload = ['ok'=>false,'message'=>"File type not allowed ({$mime}).",'request_id'=>$rid];
        json_response($payload, $formCfg['redirect_success'] ?? null, $formCfg['redirect_error'] ?? null);
        exit;
      }
      ...
      $blocked = ['php','phar','phtml','js','htm','html','svg','exe','cmd','bat','sh'];
      if (in_array($ext, $blocked, true)) {
        $payload = ['ok'=>false,'message'=>'Executable file types are not allowed.','request_id'=>$rid];
        json_response($payload, $formCfg['redirect_success'] ?? null, $formCfg['redirect_error'] ?? null);
        exit;
      }
      $rand  = random_id(12);
      $dest  = rtrim($config['paths']['uploads'], '/')."/{$formId}-{$rand}-{$safe}";
      if (!@move_uploaded_file($f['tmp_name'], $dest)) {
        $payload = ['ok'=>false,'message'=>'Failed to save uploaded file.','request_id'=>$rid];
        json_response($payload, $formCfg['redirect_success'] ?? null, $formCfg['redirect_error'] ?? null);
        exit;
      }
      $savedFiles[] = [
